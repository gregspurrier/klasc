...
describe 'Primitives for lists' do
  describe '(cons Hd Tl)' do
    it 'creates a list with Hd as the head and Tl as the tail' do
      kl_eval('(cons a ())').should be_kind_of Cons
  ...
  describe '(hd L)' do
    ...
    include_examples 'argument types', [:hd, "(cons a b)"], 1 => [:list, :dotted_pair, :empty_list]
  ...
  describe '(tl L)' do
    ...
    include_examples 'argument types', [:tl, "(cons a b)"], 1 => [:list, :dotted_pair, :empty_list]
