...
describe 'Primitives for Generic Functions' do
  ...
  describe '(lambda X Expr)' do
    ...
    it 'returns a function' do
      # There is no predicate for functions, so we can only test that it acts
      # like one.
      kl_eval('((lambda X 37) foo)').should == 37
    end 
    ...
    # Pending clarification of the spec
